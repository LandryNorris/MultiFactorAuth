...
val getDeviceDocs = MethodInfo.GetInfo(
    ...
    responseInfo = ResponseInfo(HttpStatusCode.OK,
        description = "device with this id.",
        examples = mapOf("example" to exampleDevicesList.first())),
...
val createDeviceDocs = MethodInfo.PostInfo<Unit, DeviceParams, DeviceResponse>(
    ...
    requestInfo = RequestInfo(
        description = "Parameters to create the device",
        examples = mapOf("example" to exampleCreateDeviceParams)),
    securitySchemes = setOf("jwt"),
    tags = setOf("devices")
)
...
data class GetDevicesParams(@KompendiumParam(ParamType.PATH) val id: String)
